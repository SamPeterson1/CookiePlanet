...
public class TileFactory {
	...
	public TileFactory(String packageName) {
		try {
			String[] classNames;
	...
	public Tile getTile(int id) {
		try {
			return (Tile) Class.forName(this.registations.get(Integer.valueOf(id))).getConstructor().newInstance();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private void registerClasses(String[] classNames) {
		try {
			for (String className : classNames) {
				Tile tile = (Tile) Class.forName(className).getConstructor().newInstance();
